...
class LayerLoRAAttnProcessor2_0(nn.Module):
    r"""
...
# def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None,i=0) -> torch.Tensor:
...
#     if attn_weight.size(3)==77 and i==0:
#         visal = torch.sum(attn_weight[1], dim=0)
#         heigh = int(np.sqrt(visal.size(0)))
#         if heigh == 64:
#             # j+=1
#             visal = visal.view(heigh, heigh, 77).permute(2, 0, 1)
#             # visal = torch.transpose(visal, 0, 2)
#             # resized_tensor = F.interpolate(tensor.unsqueeze(0), size=target_size, mode='bilinear', align_corners=False).squeeze(0)
#             visal = F.interpolate(visal.unsqueeze(0), size=(512,512), mode='bilinear', align_corners=True).squeeze(0)
#             visal = visal.cpu().numpy()
#             visal = np.array(visal)
            
#             # import ipdb;ipdb.set_trace()
#             for i in range(9):
#                 tempt= normalization01(visal[i])*255
#                 img = Image.fromarray(tempt.astype(np.uint8))
#                 img.save(str(i)+".png")
#         # visal = normalization01(visal)*255
        
#         # visal = torch.sum(visal,dim=0)
