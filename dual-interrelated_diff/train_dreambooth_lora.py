...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_data_root="/path/to/mvtec_anomaly_detection"
        ...
        for type_name in os.listdir(self.instance_data_root):
            if type_name not in [mvtec_name]:
                continue
            if not os.path.isdir(os.path.join(self.instance_data_root,type_name,'test')):
                continue
            for anamaly_name in os.listdir(os.path.join(self.instance_data_root,type_name,'test')):
                if anamaly_name!='good' and anamaly_name == mvtec_anamaly_name:
                    file_names=os.listdir(os.path.join(self.instance_data_root,type_name,'test',anamaly_name))
                    file_names.sort()
                    n=len(file_names)
                    n=n//3
        ...
        if type_name == 'cable':
            ...
            self.image_transforms_gt = transforms.Compose(
                [
                    ...
                    transforms.CenterCrop(64) if center_crop else transforms.RandomCrop(64),
                    transforms.ToTensor(),
                ]
            )
        else:
    ...
    def __getitem__(self, index):
        example = {}
        instance_image_blend = Image.open(self.instance_images_path[index % self.num_instance_images])
        ...
        torch.random.manual_seed(seed)
        gt = self.image_transforms_gt(mask)
        gt[gt<0.5]=0
        gt[gt>=0.5]=1
        torch.random.manual_seed(seed)
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    gt = [example["gt"] for example in examples]
    

    if has_attention_mask:
        attention_mask_blend = [example["instance_attention_mask_blends"] for example in examples]
    ...
    pixel_value_blends = pixel_value_blends.to(memory_format=torch.contiguous_format).float()
...
def main(args):
    ...
    train_dataset = DreamBoothDataset(
        ...
        mvtec_name=args.mvtec_name,
        mvtec_anamaly_name=args.mvtec_anamaly_name,
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                pixel_value_blends = batch["pixel_value_blends"].to(dtype=weight_dtype)
                pixel_value_fgs = batch["pixel_value_fgs"].to(dtype=weight_dtype)

                if vae is not None:
                    # Convert images to latent space
                    model_input_blend = vae.encode(pixel_value_blends).latent_dist.sample()
