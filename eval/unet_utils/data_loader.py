...
class MVTec_Anomaly_Detection(Dataset):
    def __init__(self, args,sample_name,length=5000,anomaly_id=None,recon=False):
        ...
        for idx,anomaly in enumerate(self.anomaly_names):
            img_path=[]
            mask_path=[]
            for img_name in os.listdir(os.path.join(self.root_dir,anomaly,'mask')):
            # for i in range(min(len(os.listdir(os.path.join(self.root_dir,anomaly,'mask'))),500)):
                img_path.append(os.path.join(self.root_dir,anomaly,'image',img_name))
                mask_path.append(os.path.join(self.root_dir,anomaly,'mask',img_name))
    ...
    def __getitem__(self, idx):
        if random.random()>0.5:
            ...
            # print(anomaly_id,len(self.mask_paths[anomaly_id]))
...
class MVTec_classification_train(Dataset):
    def __init__(self, args,sample_name):
        ...
        for idx,anomaly in enumerate(self.anomaly_names):
            for i in range(min(len(os.listdir(os.path.join(self.root_dir,anomaly,'mask'))),500)):
                self.img_paths.append(os.path.join(self.root_dir,anomaly,'image','%d.png'%i))
