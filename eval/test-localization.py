...
def test(obj_names, mvtec_path, checkpoint_path):
    ...
    for obj_name in obj_names:
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            # 可视化预测结果：
            pil_image_mask = Image.fromarray((out_mask_cv * 255).astype(np.uint8), mode='L')
            pil_image = transforms.ToPILImage()(sample_batched['image'][0][[2,1,0],:,:])
            draw = ImageDraw.Draw(pil_image)
            draw.bitmap((0, 0), pil_image_mask, fill="red")   
            # pil_image.save('result/'+obj_name+'_'+str(i_batch)+"_mask.jpg") 
            
            pil_image_gt = Image.fromarray((true_mask_cv[:,:,0]  * 255).astype(np.uint8), mode='L')
            pil_image_gt = pil_image_gt.convert('RGB')
            width = 256*2
            height = 256
            result = Image.new('RGB', (width, height))
            result.paste(pil_image, (0, 0))
            result.paste(pil_image_gt, (256, 0))
            result.save('result/'+obj_name+'_'+str(i_batch)+"_mask.jpg")
            # 
    ...
    print("F1-max Pixel mean: " +str(np.mean(obj_f1_pixel_list)))
