...
from .models.attention_processor import (
    ...
    LoRAXFormersAttnProcessor,Attention,
    SlicedAttnAddedKVProcessor,AttnProcessor2_0,
...
class UNet2DConditionLoadersMixin:
    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
        ...
        is_lora = all("lora" in k for k in state_dict.keys()) or any("temporal" in k for k in state_dict.keys())
        ...
        if is_lora:
            ...
            for key, value in state_dict.items():
                ...
                if 'attn1' in key:
                    if 'temporal' in key:
                        if 'condition_mlp' in key:
                            attn_processor_key, sub_key = ".".join(key.split(".")[:-4]), ".".join(key.split(".")[-3:])
                            lora_grouped_dict[attn_processor_key][sub_key] = value
                        elif 'temporal_a' in key:
                            attn_processor_key, sub_key = ".".join(key.split(".")[:-2]), ".".join(key.split(".")[-1:])
                            lora_grouped_dict[attn_processor_key][sub_key] = value
                        else:
                            attn_processor_key, sub_key = ".".join(key.split(".")[:-3]), ".".join(key.split(".")[-2:])
                            lora_grouped_dict[attn_processor_key][sub_key] = value
                    else:
                        attn_processor_key, sub_key = ".".join(key.split(".")[:-5]), ".".join(key.split(".")[-4:])
                        lora_grouped_dict[attn_processor_key][sub_key] = value
                    ...
                    if 'temporal' in key:
                        if 'condition_mlp' in key:
                            ...
                            attn_processor_key, sub_key = ".".join(key.split(".")[:-1]), ".".join(key.split(".")[-1:])
                            lora_grouped_dict[attn_processor_key][sub_key] = value
                        else:
            ...
            for key, value_dict in lora_grouped_dict.items():
                # import ipdb;ipdb.set_trace()
                rank = value_dict["to_k_lora.0.down.weight"].shape[0]
                hidden_size = value_dict["to_k_lora.0.up.weight"].shape[0]
                ...
                if isinstance(
                    ...
                    cross_attention_dim = value_dict["to_k_lora.0.down.weight"].shape[1]
        ...
        for name,value in attn_processors.items():
            if name.endswith("attn1.processor"):
                unet_lora_attn_procs[name] = ReferenceOnlyAttnProc(
                    value, enabled=name.endswith("attn1.processor"), name=name
                )
        
        attn_processors.update(unet_lora_attn_procs)
