...
class LoRAAttnProcessor2_0(nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, rank=4, network_alpha=None):
        ...
        self.temporal_condition_mlp = nn.Sequential(
            nn.Linear(hidden_size, 320),
            # nn.ReLU(),
            nn.Linear(320, hidden_size)
        )
        # [0.0183]
        # self.temporal_k_con = torch.nn.Linear(in_features=hidden_size, out_features=hidden_size)
        # self.temporal_v_con = torch.nn.Linear(in_features=hidden_size, out_features=hidden_size)
        # self.temporal_o_con = torch.nn.Linear(in_features=hidden_size, out_features=hidden_size)
        self.temporal_a = torch.nn.Parameter(torch.Tensor([0]))
        

    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, scale=1.0,**kwargs):
        mode = kwargs['mode']
        # print(mode)
        if mode == 'w':
            # print(encoder_hidden_states.shape)
            # self.att;only blend; mask trick ;
            residual = hidden_states
            batch_size, sequence_length, _ = (
                hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
            )
            inner_dim = hidden_states.shape[-1]
            query = attn.to_q(hidden_states) + scale * self.to_q_lora[0](hidden_states)
            key = attn.to_k(encoder_hidden_states) + scale * self.to_k_lora[0](encoder_hidden_states)
            value = attn.to_v(encoder_hidden_states) + scale * self.to_v_lora[0](encoder_hidden_states)
            head_dim = inner_dim // attn.heads
            query = query.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            key = key.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            value = value.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
            ...
            # linear proj
            hidden_states = attn.to_out[0](hidden_states) + scale * self.to_out_lora[0](hidden_states)
            # dropout
            hidden_states = attn.to_out[1](hidden_states)
            ...
            for i in range(self.frames):
                ...
                # if 'condition_encoder_state' in kwargs and i == 0:
                #     condition_encoder_state = kwargs['condition_encoder_state']
                #     con_k = self.temporal_k_con(condition_encoder_state)
                #     con_v = self.temporal_v_con(condition_encoder_state)
                #     con_k = con_k.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
                #     con_v = con_v.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
                #     con_hidden_states = F.scaled_dot_product_attention(
                #         query, con_k, con_v, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
                #     )
                #     con_hidden_states = con_hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
                #     con_hidden_states = con_hidden_states.to(query.dtype)
                ...
                hidden_states = attn.to_out[1](hidden_states)
                # if 'condition_encoder_state' in kwargs and i == 0:
                #     hidden_states = (1-self.temporal_a) * hidden_states + self.temporal_a * con_hidden_states
